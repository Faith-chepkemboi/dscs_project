...
import hashlib     # To generate SHA-1 hashes for file content and commit objects
import shutil      # To copy directory structures (used for cloning)
import time        # To track commit timestamps


class Repository:
    #__init__ initializes the repository structure:
    #repo_dir: The main directory for the repository (default is .dscs).
    #head_file is File to track the current branch (similar to Git's HEAD).
    #refs_dir is  Directory to store branch references.
    # objects_dir is Directory to store file and commit objects.
...
#Initializing the Repository
    ...
    def merge(self, branch_name):
        ...
        if current_commit == branch_commit:
            ...
            # For simplicity, merge by just adding the commit to the history
    ...
    def clone(self, target_dir):
        ...
        if not os.path.exists(self.repo_dir):
            print("Repository not initialized. Nothing to clone.")
            return
